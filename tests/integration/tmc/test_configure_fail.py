...
@scenario("features/tmc_configure_fail.feature", "Configure the mid telescope subarray using TMC")
...
def an_telescope_subarray(
    set_up_subarray_log_checking_for_tmc, base_configuration: conf_types.ScanConfiguration,  subarray_allocation_spec: fxt_types.subarray_allocation_spec,
...
@then("the subarray must be in the SCANNING state")
def the_subarray_must_be_in_the_scanning_state(
    ...
    """the subarray must be in the SCANNING state."""
    import ipdb
    ipdb.set_trace()
    tel = names.TEL()
