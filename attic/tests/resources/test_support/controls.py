...
class pilot:
    ...
    def to_be_composed_out_of(
        self, dishes, file="resources/test_data/OET_integration/example_allocate.json"
    ):
        ...
        self.state = "Composed"
        LOGGER.info(
            "_________Sdp block from composed function_______"
            + str(self)
            + str(sdp_block)
        )
    ...
    def and_configure_scan_by_file(
        ...
        def config(file, sdp_block):
            ...
            LOGGER.info(
                "___________Input file in configure_oet_____________" + str(file)
            )
            self.state = "Configuring"
            self.SubArray.configure_from_file(file, 6, with_processing=False)

        LOGGER.info(
            "___________SDP block from configure_oet_____________" + str(sdp_block)
        )
...
def run_a_config_test():
    ...
    try:
        ...
        if resource("ska_mid/tm_subarray_node/1").get("obsState") == "IDLE":
            ...
            raise Exception(
                "failure in configuring subarry not configured, resources are released and put in standby"
            )
