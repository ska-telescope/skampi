...
def _is_tarball(fname):
    return (
        ...
        or fname.endswith(".tar.xz")
        or fname.endswith(".tar.bz2")
    )
...
class Report:
    ...
    def add_log(self, fname, log, source=None, revision=None):
        ...
        if log_date is not None:
            ...
            if revision is not None and (
                revision not in self.revisions or self.revisions[revision] < log_date
            ):
            ...
            for cfr in classifiers.classifiers:
                ...
                if not cfr_matches:
                    continue
                self.add_cfr_matches(
                    fname, source, log, log_date, cfr, cfr_matches, revision
                )
    ...
    def add_tarball(self, tar, source, revision=None):
        while True:
            ...
            with tar.extractfile(info) as f:
                try:
                    self.add_log(
                        info.name, logs.collect_file(info.name, 1, f), source, revision
                    )
                except Exception:
                    traceback.print_exc()
    ...
    def add_from_gitlab(self, uri, header, project, search, job_names, artifact):
        ...
        for pip in pips:
            ...
            for job in (job for job in jobs if job.name in job_names):
                ...
                try:
                    self.add_file_or_uri(
                        f"{uri}/{project}/-/jobs/{job.id}/artifacts/raw/{artifact}?inline=false",
                        pip.sha,
                    )
                except Exception:
                    traceback.print_exc()
    ...
    def build_maps(self):
        ...
        self.files_per_cfr = {
            cfr.skb: {
                fname
                for fname, matches in self.matches_per_file.items()
    ...
    def make_overview_table(self, f, revision=None):
        ...
        if file_count > 0:
            ...
            print(
                f"Date range: {self.file_date[files_sorted[0]]} to {self.file_date[files_sorted[-1]]}\n",
                file=f,
            )
        ...
        rows = []
        for cfr in sorted(
            cfrs_sorted, key=lambda cfr: len(self.files_per_cfr[cfr.skb]), reverse=True
        ):
            ...
            if revision is not None:
                ...
                matches = [
                    cfr_match
                    for cfr_match in matches
                    if cfr_match["revision"] == revision
                ]
            ...
            if matches:
                ...
                if any(
                    cfr_match["log_id"] == last_match["log_id"]
            ...
            if files_with_matches:
                ...
                cfr_intersection_changes = sorted(
                    [
                        (
                            cfr2,
                            cfr_intersection[cfr2.skb] / len(files_with_matches)
                            - len(self.files_per_cfr[cfr2.skb]) / file_count,
                        )
                        for cfr2 in classifiers.classifiers
                        if len(self.files_per_cfr[cfr2.skb]) > 0
                        and cfr2.skb != cfr.skb
                        and (not cfr.taints or not cfr2.skb.startswith("TAINT"))
                    ],
                    key=lambda skb_diff: skb_diff[1],
                    reverse=True,
                )

                other_skb_changes = ", ".join(
                    f":ref:`{cfr2.skb} <{cfr2.skb}>`: {change*100:+.1f}%"
    ...
    def make_log_block(self, f, log, highlight_lines):
        ...
        to_highlight = sorted(
            [i for i, l in enumerate(log) if l.get("time") in highlight_lines]
        )
        if not to_highlight:
        ...
        for i, l in enumerate(log):
            ...
            if i < to_highlight[0] - self.context_lines:
                if not skipping:
                    lines_out.append(
                        f"  ... {to_highlight[0]-i-self.context_lines} lines skipped ..."
                    )
                    skipping = True
                continue
        ...
        if highlight_out:
            print(
                f"  :emphasize-lines: {','.join(str(i) for i in highlight_out)}", file=f
            )
        print(file=f)
        for s in lines_out:
            print(s, file=f)
    ...
    def write_logs(self, directory):
        ...
        for log_id, cfr_match in logs_to_generate.items():
            with open(
                pathlib.Path(directory, f"log-{log_id}.txt"), "w", encoding="utf-8"
            ) as f:
    ...
    def make_pod_timing_table(self, f):
        ...
        pod_timings_avg = {
            pod: {cont: sum(ts) / len(ts) for cont, ts in pod_conts.items()}
        ...
        pods_sorted = sorted(
            pod_timings_avg.items(), key=lambda pod_map: max(pod_map[1].values())
        )
        rows = []
        for pod, cont_avgs in pods_sorted:
            ...
            for cont, avg in sorted(
                cont_avgs.items(), key=lambda cont_avg: cont_avg[1]
            ):
