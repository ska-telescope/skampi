...
def check_going_into_abort():
    ...
    resource("ska_mid/tm_subarray_node/1").assert_attribute("obsState").equals(
        ["IDLE", "SCANNING", "CONFIGURING", "READY", "RESETTING"]
    )
    resource("ska_mid/tm_subarray_node/1").assert_attribute("State").equals("ON")
...
class WaitAbort:
    def __init__(self):
        self.the_watch = watch(resource("ska_mid/tm_subarray_node/1")).for_a_change_on(
            "obsState"
        )
...
class WaitResetting:
    def __init__(self):
        ...
        self.the_watch1 = watch(resource("mid_sdp/elt/subarray_1")).for_a_change_on(
            "obsState"
        )
        resource("ska_mid/tm_subarray_node/1").assert_attribute("obsState").equals(
            "ABORTED"
    ...
    def wait(self, timeout):
        logging.info(
            "ObsReset command invoked. Waiting for obsState to change to Resetting"
        )
