...
sys.path.appendos.path.dirname(os.path.abspath(__file__))
...
if arguments["--gitlab"] is not None:
    ...
    if (
        arguments["--gitlab-project"] is None
        or arguments["--gitlab-job"] is None
...
with open("overview.rst", "w", encoding="utf-8") as f:
    ...
    for rev, date in sorted(
        report.revisions.items(), key=lambda rev_date: rev_date[1], reverse=True
    ):
