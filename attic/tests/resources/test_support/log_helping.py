...
class DeviceLogging:
    def __init__(self, implementation="TracerHelper"):
        if implementation == "TracerHelper":
            ...
            self._shim = {
                "set_logging_level": self.implementation.set_logging_level,
            ...
            raise Exception(
                "unknown implentation of Device logging {}".format(implementation)
            )
...
class DeviceLoggingImplWithDBDirect:
            ...
            if self.local:
                es = Elasticsearch(
                    [
                        {
                            "host": "{}-{}".format(
                                "elastic-logging", os.environ.get("HELM_RELEASE")
                            ),
                            "port": 9200,
                        }
                    ]
                ...
                es = Elasticsearch([{"host": "192.168.93.94", "port": 9200}])
        else:
    ...
    def _run_query(self):
        search = (
            self.search.query(self._qeury_by_time_window())
                ...
                self.dict_results.append(
                    {
                        ...
                        "device": self.containers_to_devices[
                            hit.kubernetes.container_name
                        ],
                    }
                )
    ...
    def _format_log_data(self, log):
        log = " reception date: {} device/comp: {} message: '{}' container: {} ".format(
