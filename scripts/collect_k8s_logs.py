...
if all(target is None for target in [pp_target, dump_target, tests_target, eval_target, eval_json_target]):
    pp_target = '-'
...
for dump_file in make_target(dump_target, f'Dumping JSON to {dump_target}...'):
    for line in lines:
        print(json.dumps( { **line, 'time': logs.render_date(line['time']) }),
              file=dump_file)
...
for eval_file in make_target(eval_target, f'Writing evaluation to {eval_target}...'):
    ...
    for trigger in triggers:
        ...
        test = trigger['test']
        print(f"{'!! ' if cfr.taints else ''}{cfr.skb} {cfr.message}  "
              f"[{len(matched)} instance{'s' if len(matched)>1 else ''} "
              f"in {test['file']} {test['name']} {test['status']}"
              f"{', possibly more' if cfr.only_once else ''}]", file=eval_file)
...
for eval_json_file in make_target(eval_json_target,
                                  f'Writing JSON evaluation to {eval_json_target}...'):
    ...
    for trigger in triggers:
        outputs.append({
            'source': arguments['<ns>'],
            'test': {
                k: v for k,v in trigger['test'].items()
